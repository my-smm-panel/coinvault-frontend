...
class _PaymentwallScreenState extends State<PaymentwallScreen> {
  ...
  Future<void> _load() async {
    ...
    final data = await PaymentwallService.instance.fetchOffers();
    if (!mounted) return;
    setState(() {
      _loading = false;
      if (data == null) {
  ...
  List<PaymentwallOffer> get _filtered {
    var list = _offers;
    if (_filter == 'Paymentwall') {
      list = list.where((o) {
        final p = o.provider.toLowerCase();
        return p == 'paymentwall' || p == 'payment_wall' || p == 'payment wall';
      }).toList();
  ...
  Future<void> _startOffer(PaymentwallOffer offer) async {
    final clickUrl = await PaymentwallService.instance.startOffer(offer.id);
    if (!mounted) return;
    if (clickUrl != null && clickUrl.isNotEmpty) {
      // Validate URL is HTTPS before opening
      final uri = Uri.tryParse(clickUrl);
      if (uri != null && uri.scheme == 'https') {
        // Open in external browser — preserves Paymentwall tracking chain
        // CoinVault → Paymentwall → Advertiser
        try {
  ...
  void _openDetail(PaymentwallOffer offer) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PaymentwallDetailScreen(
          offerId: offer.id,
  ...
  Widget _filterChips() {
    final chips = <String>[
      ...
      'Paymentwall',
      ..._providers.where((p) =>
          p.toLowerCase() != 'paymentwall' &&
          p.toLowerCase() != 'payment_wall' &&
